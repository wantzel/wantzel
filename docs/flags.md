...
**Two arguments and one option — the whole interface.**
...
| **stripping the build path** | nothing to strip — a runtime message carries at most the last two path segments (`src/ui/main.wz:412`), so the same source gives the same binary built from any path |
