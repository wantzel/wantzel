...
long genwin(){
    ...
    gen("\n");
    gen("\n");
    gen("// connect: Winsock's sockaddr_in is Linux's byte for byte (see __wbind), and the\n");
    gen("  // socket from __wsock is blocking, so a failure is a refusal, not EINPROGRESS.\n");
    gen("function __wconnect(fd: int; sa: int; sln: int): int;\n");
    gen("var r: int;\n");
    gen("begin\n");
    gen("  if not __wissock(fd) then return 0 - 9;\n");
    ...
    gen("end;\n");
    gen("\n");
    gen("// kill: signal 0 asks only whether the process exists (OpenProcess plus\n");
    gen("  // GetExitCodeProcess = STILL_ACTIVE); any other signal ends it with TerminateProcess,\n");
    gen("  // exit code 128 + sig as a shell would report it.  ESRCH when there is no such process.\n");
    gen("function __wkill(pid: int; sig: int): int;\n");
    gen("var h, r: int;\n");
    gen("begin\n");
    gen("  if sig = 0 then\n");
    gen("  begin\n");
    gen("    h := winapi(\"kernel32.dll\", \"OpenProcess\", 0x1000, 0, pid);   // QUERY_LIMITED_INFORMATION\n");
    gen("    if h = 0 then return 0 - 3;\n");
    gen("    r := winapi(\"kernel32.dll\", \"GetExitCodeProcess\", h, addr(__wexitc[0]));\n");
    gen("    __wdummy := winapi(__K_CLOSEHANDLE, h);\n");
    gen("    if band(r, 0xFFFFFFFF) = 0 then return 0 - 3;\n");
    gen("    if __wg32(__wexitc, 0) = 259 then return 0;   // STILL_ACTIVE\n");
    gen("    return 0 - 3;\n");
    gen("  end;\n");
    gen("  h := winapi(\"kernel32.dll\", \"OpenProcess\", 1, 0, pid);   // PROCESS_TERMINATE\n");
    gen("  if h = 0 then return 0 - 3;\n");
    gen("  r := winapi(\"kernel32.dll\", \"TerminateProcess\", h, 128 + sig);\n");
    gen("  __wdummy := winapi(__K_CLOSEHANDLE, h);\n");
    gen("  if band(r, 0xFFFFFFFF) = 0 then return 0 - 1;   // EPERM\n");
    gen("  return 0;\n");
    gen("end;\n");
    gen("\n");
    gen("// nanosleep: the timespec at ts (tv_sec, then tv_nsec, each 64-bit little-endian),\n");
    gen("  // rounded UP to whole milliseconds because Sleep counts in ms.  The low four bytes of\n");
    gen("  // each field are read: 2^31 seconds is more than any caller asks for.\n");
    gen("function __wsleep(ts: int): int;\n");
    gen("var s, ns, i: int;\n");
    gen("begin\n");
    gen("  s := 0; ns := 0; i := 3;\n");
    gen("  while i >= 0 do\n");
    gen("  begin\n");
    gen("    s := bor(s shl 8, peek(ts + i));\n");
    gen("    ns := bor(ns shl 8, peek(ts + 8 + i));\n");
    gen("    i := i - 1;\n");
    gen("  end;\n");
    gen("  __wdummy := winapi(__K_SLEEP, s * 1000 + (ns + 999999) div 1000000);\n");
    gen("  return 0;\n");
    gen("end;\n");
    gen("\n");
    ...
    gen("    return b;\n");
    gen("  end;\n");
    gen("  if nr = 35 then return __wsleep(a);   // nanosleep\n");
    ...
    gen("  if nr = 39 then return band(winapi(\"kernel32.dll\", \"GetCurrentProcessId\"), 0xFFFFFFFF);   // getpid\n");
    gen("  if nr = 62 then return __wkill(a, b);   // kill\n");
